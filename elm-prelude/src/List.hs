...
-}
repeat :: Int -> a -> List a
repeat n value = repeatHelp [] n value

repeatHelp :: List a -> Int -> a -> List a
repeatHelp result n value =
  if n <= 0
    then result
    else repeatHelp (value : result) (n - 1) value
...
range lo hi = rangeHelp lo hi []

rangeHelp :: Int -> Int -> List Int -> List Int
rangeHelp lo hi list =
  if lo <= hi
    then rangeHelp lo (hi - 1) (hi : list)
    else list
...
foldr fn acc ls = foldrHelper fn acc 0 ls
            ...
            c:r3 ->
              case r3 of
                ...
                d:r4 ->
                  let res =
                        if ctr > 500
                          then foldl fn acc (reverse r4)
                          else foldrHelper fn acc (ctr + 1) r4
                   in fn a (fn b (fn c (fn d res)))
...
-}
length :: List a -> Int
length xs = foldl (\_ i -> i + 1) 0 xs

{-| Reverse a list.
    reverse [1,2,3,4] == [4,3,2,1]
-}
reverse :: List a -> List a
reverse list = foldl (:) [] list
...
sum numbers = foldl (+) (fromInteger 0) numbers

{-| Get the product of the list elements.
    product [1,2,3,4] == 24
-}
product :: Number number => List number -> number
product numbers = foldl (*) (fromInteger 1) numbers
...
map3 f =
  let go acc [] _ _               = reverse acc
      go acc _ [] _               = reverse acc
      go acc _ _ []               = reverse acc
      go acc (x:xs) (y:ys) (z:zs) = go (f x y z : acc) xs ys zs
   in go []
...
map4 ::
  ...
  -> List result
map4 f =
  let go acc [] _ _ _                    = reverse acc
      go acc _ [] _ _                    = reverse acc
      go acc _ _ [] _                    = reverse acc
      go acc _ _ _ []                    = reverse acc
      go acc (x:xs) (y:ys) (z:zs) (t:ts) = go (f x y z t : acc) xs ys zs ts
   in go []
...
map5 ::
  ...
  -> List result
map5 f =
  let go acc [] _ _ _ _ = reverse acc
      go acc _ [] _ _ _ = reverse acc
      go acc _ _ [] _ _ = reverse acc
      go acc _ _ _ [] _ = reverse acc
      go acc _ _ _ _ [] = reverse acc
      go acc (x:xs) (y:ys) (z:zs) (t:ts) (u:us) =
        go (f x y z t u : acc) xs ys zs ts us
   in go []
...
takeFast ctr n list =
  if n <= 0
    then []
           ...
           (_, x:y:z:w:tl) ->
             if ctr > 1000
               then x : y : z : w : (takeTailRec (n - 4) tl)
               else x : y : z : w : (takeFast (ctr + 1) (n - 4) tl)
           _ -> list

takeTailRec :: Int -> List a -> List a
takeTailRec n list = reverse (takeReverse n list [])
...
takeReverse n list kept =
  if n <= 0
    then kept
           ...
           []   -> kept
           x:xs -> takeReverse (n - 1) xs (x : kept)
